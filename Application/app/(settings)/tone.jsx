...
const Tone = () => {
    ...
    const handleSaveChange = async () => {
        ...
        try {
            const res = await axios.put(`${API_URL}/api/v1/preference/${decryptedToken.userID}`, {preferredTone: activeRadioButton},{ 
    ...
    const getUserPreference = async () => {
        ...
        try {
            ...
            setActiveRadioButton(preference.data.data.preferredTone);
            return;
        } catch (error) {
  ...
  return (
    <>  
        ...
        <SafeAreaView className='min-w-screen min-h-screen bg-background px-8 pt-32 gap-8'>
            ...
            <Modal visible={loading} transparent={true}>
                <View className='w-screen h-screen items-center justify-center' style={{backgroundColor: 'rgba(0,0,0,0.5)'}}>
                    <View className='w-4/5 h-auto bg-white items-center gap-8 py-12 justify-center rounded-xl'>
                        <Text className='text-2xl font-bold'>Changing Preferred Tone</Text>
                        <ActivityIndicator color={'#0818A8'} size={32}/>
            ...
            <View className='w-full h-auto flex-col mb-4 gap-2'>
                <Text className='text-xl font-bold'>Tone</Text>
                <Text>Select your preferred AI's tone.</Text>
            </View>
            ...
            <View className='w-full h-auto flex-col gap-6 px-2'>
                <TouchableOpacity className='w-full h-auto flex-row' onPress={() => setActiveRadioButton('formal')}>
                    <Text className='text-lg font-bold flex-1'>Formal</Text>
                    {activeRadioButton === 'formal' ? (<Text><Ionicons name="radio-button-on" size={24} color="black" /></Text>) : (<Text><Ionicons name="radio-button-off-sharp" size={24} color="black" /></Text>)}
                </TouchableOpacity>
                <TouchableOpacity className='w-full h-auto flex-row' onPress={() => setActiveRadioButton('casual')}>
                    <Text className='text-lg font-bold flex-1'>Casual</Text>
                    {activeRadioButton === 'casual' ? (<Text><Ionicons name="radio-button-on" size={24} color="black" /></Text>) : (<Text><Ionicons name="radio-button-off-sharp" size={24} color="black" /></Text>)}
                </TouchableOpacity>
                <TouchableOpacity className='w-full h-auto flex-row' onPress={() => setActiveRadioButton('soft spoken')}>
                    <Text className='text-lg font-bold flex-1'>Soft Spoken</Text>
                    {activeRadioButton === 'soft spoken' ? (<Text><Ionicons name="radio-button-on" size={24} color="black" /></Text>) : (<Text><Ionicons name="radio-button-off-sharp" size={24} color="black" /></Text>)}
                </TouchableOpacity>
                <TouchableOpacity className='w-full h-auto flex-row' onPress={() => setActiveRadioButton('strict')}>
                    <Text className='text-lg font-bold flex-1'>Strict</Text>
                    {activeRadioButton === 'strict' ? (<Text><Ionicons name="radio-button-on" size={24} color="black" /></Text>) : (<Text><Ionicons name="radio-button-off-sharp" size={24} color="black" /></Text>)}
                </TouchableOpacity>
            </View>
