  ...
  return (
    <TouchableOpacity onPress={handlePress} activeOpacity={0.8}>
      ...
      >
        ...
        <View className="w-full px-2 py-3 pr-4 flex flex-row border-t-[1px] border-gray-200">
          <View className="flex-1 flex-row gap-2 items-center justify-start">
            <View className="w-6 h-6 rounded-full">
              <Image
                source={profileIcons[guide.posterInfo.profileIcon]}
                className="max-w-full h-full rounded-full"
              />
            </View>
            <Text
              className="text-md text-gray-400 font-semibold flex-1"
              numberOfLines={1}
              ellipsizeMode="tail"
            >
              {!guide ? "sample username" : `${guide.posterInfo.name}`}
            </Text>
          </View>
...
export default CardRecentGuidePerTypeHorizontal