...
deleteGuide: async (guide, imageIDs) => {
    try {
        ...
        for (const { public_id, url } of imageIDs) {
            ...
            try {
                const deleteRes = await axios.post(`${import.meta.env.VITE_URI}guide/deleteImage`, {
        ...
        const guideDeleteRes = await axios.post(`${import.meta.env.VITE_URI}guide/${guide._id}`);
  ...
  try {
    ...
    for (const public_id of deletedStepImageIds) {
      try {
        ...
        console.error(`Failed to delete explicitly removed image with ID ${public_id}:`, err.message);
      }
    }
    ...
    for (const img of excessStepImgs) {
      if (img?.public_id) {
        try {
          ...
          console.error(`Failed to delete excess step image with ID ${img.public_id}:`, err.message);
        }
      }
    }
      ...
      if (file) {
        ...
        if (stepFilesData[i]?.public_id) {
          try {
            ...
            console.error(`Failed to delete previous step image at index ${i}:`, err.message);
          }
        }
